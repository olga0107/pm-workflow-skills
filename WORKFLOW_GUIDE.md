...
flowchart LR
  ...
  P -- "interactive-hifi 可交互原型" --> X["pm-prototype-deliver"]
  D -- "方案已确认，需体验交付" --> X
...
| 可交互原型交付 | `pm-prototype-deliver` | 方案或 PRD 已确认，需要相关方上手操作和体验方案（`prototypeTarget: interactive-hifi`）；支持方案 → 原型 → PRD 对齐的原型先行顺序 | 不做方向探索与低保真试错，不做静态阅读原型（与 `pm-prd-html` 互斥），不替代最终 UI 设计稿；开工必须过参照物 / 设计专项技能 / 真实数据三检查，真实数据未就位不进入视觉打磨 | 高保真可交互原型、公开发布链接、文档嵌入、原型与文档的对齐回写 |
...
### 3.3.1 C3.5 PRD → UX 原型交接契约

当需求要求在 UI 设计前评估页面效果时，`pm-prd-write` 与 `pm-prd-html` 之间必须传递同一份可追溯事实，不允许由 HTML 端补猜：

| 交接项 | PRD 必须提供 | HTML 必须验证 / 呈现 |
|---|---|---|
| 页面入口与用户任务 | 页面 ID、入口、主任务、退出 / 返回语义 | 目录定位、状态故事和页面入口说明 |
| 页面 Schema | 视口、页面骨架、信息区、组件层级、固定 / 滚动区域 | 对应 DOM / SVG 结构和移动端视窗 |
| 状态架构 | 进入条件、状态差异、主操作、系统结果、下一状态、异常出口 | 每个 required 状态的静态 SVG 和可读上下文 |
| 内容契约 | 用户可见文案、字段、标签、按钮、placeholder、空态、错误、反馈、代表性数据 | 内容覆盖台账；缺项标记为待确认 / 阻塞，不静默补造 |
| 视口契约 | 逻辑尺寸、页面长度、滚动 / 吸顶 / 吸底、状态锚点、密度和可访问性约束 | 390×844 基准及 320 / 375 / 430 宽度检查；长页真实滚动 |
| 设计基线 | 中性 Token、组件语义、参考资产和 `ux-prototype` / `wireframe` 档位 | 单文件内嵌 SVG；不把 HTML 变成最终 UI 定稿 |
| 静态交付 | 禁止外部资源、禁止运行时唯一渲染、SVG 可独立解析 | `validate-html-prototypes.py`、脚本语法检查和浏览器 / 无脚本验收证据 |

`prototypeReadiness` 只表示“PRD 事实足够进入编译”，不表示 HTML 已通过视觉验收；HTML 生成完成也不表示已达到设计交接，必须同时保留静态检查、内容覆盖和视觉阅读结果。
...
需要团队可视化评审时，将已确认且通过 `prototypeReadiness` 门禁的 PRD 交给 `pm-prd-html`。它必须把正式 PRD 编译成可顺着阅读的结构，并在同一 HTML 内提供按章节回看的完整规格；固定可收起侧栏用于定位，原型默认使用 `ux-prototype` 的中性视觉基线，只有显式 `wireframe` 才使用黑白灰线框；长页在真实手机视窗内滚动，状态切换定位到 PRD 已声明的用户视口锚点，原型外再展示需求、规则和验收上下文。每个状态的 SVG 必须直接存在于初始 DOM，脚本仅提供渐进增强；受限预览禁用 JavaScript 后，仍应能阅读页面状态、内容和关键上下文。命中内容完整型 UX 原型时，原型中的用户可见内容必须全部可回溯到内容契约，并报告未覆盖 / 待确认 / 阻塞项；复杂体验按页面地图、状态架构、组件行为、端与布局矩阵和覆盖计划编译代表故事，并可核对成图与豁免依据。它只编译和重组内容，发现产品逻辑、状态迁移或重大取舍问题时必须回到 `pm-prd-write`，不在 HTML 中另立口径。PRD 确认后，可用 `pm-update-write` 生成评审通知、上线说明、运营口径或更新日志。

### 3.4A 既有系统规则迭代 → 收尾闭环

已有功能的规则 / 角色 / 指标 / 结算改造，不应直接套用“新需求从 0 到交付”。在需求定义或分级确认后，按[需求迭代闭环与收尾协议](./shared-references/需求迭代闭环与收尾协议.md)补齐四张最小表：

1. **规则变更台账**：旧规则、新规则、明确不变项、生效范围、历史边界；
2. **影响地图**：页面、写入、派生计算、触发链路、后台 / 运营、测试 / 观测、文档 / 发布；
3. **时序与归属表**：事实发生点、冻结点、重算触发、幂等、失败恢复和同主体多角色去重；
4. **交付闭环表**：实现、验收、稳定知识、需求包、交付状态、发布状态和更新说明。

发版前使用 `pm-quality-audit` 的 `release-closure` 模式或等价专项检查，必须区分：
...
“可提审”不等于“已上线”；测试环境造数、静态检查和用户验收也不能替代生产发布确认。
...
1. 先读取历史需求包、稳定知识和实现入口，建立旧规则基线；
2. 用 `pm-requirement-define` 先确认业务原则、冻结点、历史边界和非目标，不把技术方便当成业务结论；
3. 用 `pm-requirement-grade` 扫描状态机、数据写入、定时 / 批处理、权限、回滚和生产影响；
4. 需要规格时，`pm-prd-write` 补齐变更台账、影响地图、时序 / 归属、幂等 / 失败恢复和验收矩阵；
5. 实现完成后按影响地图逐项验证，并将运行证据写入实现记录 / 验收清单；
6. 发版前用 `pm-quality-audit` 做收尾审查，修复旧口径残留、可观测性缺口和文档状态冲突；
7. 验收通过后同步稳定知识；明确生产发布后再补发布状态，并用 `pm-update-write` 输出统一口径。

不命中规则、时序、历史或跨模块条件的普通文案 / 样式修改，不必展开全部四张表。
