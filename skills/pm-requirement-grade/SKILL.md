...
分级不是一次性标签。若在 PRD、技术预验证、实现或验收阶段发生以下变化，必须重新回到本 skill 评级：

- 新增写入、结算、迁移、双写、异步批处理、并发事务或外部依赖；
- 改变角色、权限、核心状态机、历史数据边界、资金 / 权益或不可快速回滚范围；
- 从只读展示变为会影响累计指标、等级、奖励、通知或历史结果；
- 发现原先“L2 可控”的假设不成立，需引入复杂兼容或生产切换。

重新分级时保留旧等级及其依据，但明确 `superseded`，重新检查下游门禁；不能因为代码已经开始或用户已认可旧 PRD 就沿用旧等级。技术复杂度本身不是升档理由，只有它对应的真实影响、失败后果和恢复成本才决定等级。
...
6. **判断页面内容交付门槛**：如果需要在设计前评估完整页面效果，或设计不应补齐用户可见信息，除常规复杂体验信号外，标记“内容完整型 UX 原型”，要求下游建立页面内容契约、UX 原型交付配置和就绪门禁；这不改变风险等级。
7. **判断静态交付门槛**：如果目标预览环境可能限制脚本、外部资源或动态 DOM，要求下游将 `staticDelivery` 设为单文件 HTML + 静态 SVG + 无 JS 可读降级；这属于交付约束，不改变风险等级。
8. **输出标准化路由包**：按最终等级输出固定结构；同一次输出中必须先给评级结论，再给门禁要求、体验交付复杂度和下一步交接，不在本 skill 内扩写下游正式产物。
...
- 2026-08-26 · v0.2 · 将 UX 原型档位、静态交付和 `prototypeReadiness` 纳入体验交付路由，避免分级与 PRD / HTML 标准断链。
...
| UX 原型交付与静态预览约束 | 已清楚/待补/不适用 | 原型档位、真实性要求、是否需要页面 Schema / 代表性数据 / 静态 SVG 和就绪门禁 |
| 数据读写与权限 | 已清楚/待补 |  |
| 验收与回滚 | 已清楚/待补 |  |
...
- 若命中“内容完整型 UX 原型”，必须把页面内容契约、页面 × 状态 × 内容覆盖、代表性数据、UX 原型交付配置和 `prototypeReadiness` 列入交接清单；若目标环境受限，还必须列入静态 HTML / SVG 降级要求；不得只写“需要原型”或“交给设计补齐”。
